...
F0BasicTimer::F0BasicTimer(uintptr_t timer_addr, IRcc *rcc) {
  timer_memory = reinterpret_cast<F0BasicTimerMemory *>(timer_addr);
...
Rcc_Timer F0BasicTimer::get_timer_rcc() {
  ...
  if (timer_memory == reinterpret_cast<F0BasicTimerMemory *>(Timer_Peripheral::TIMER6))
    timer = Rcc_Timer::TIMER6;
  else if (timer_memory == reinterpret_cast<F0BasicTimerMemory *>(Timer_Peripheral::TIMER7))
